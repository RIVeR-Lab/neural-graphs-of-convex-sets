...
#   1) Quadrotor          (max_paths = 10, hardcoded in quadrotor/scripts/benchmark_quadrotor.py)
...
#   planning_through_contact/results/benchmark/sugar_box_<N>/results_table.pdf
#   planning_through_contact/results/benchmark/tee_<N>/results_table.pdf
#
...
run_step "[1/3] Quadrotor — table (CPU, max_paths=10)" \
    "bash quadrotor/scripts/quadrotor_benchmark.sh --device cpu"
...
run_step "[2/3] Planar pushing sugar_box — table (CPU, max_paths=100)" \
    "bash planning_through_contact/scripts/ptc_benchmark.sh --box --skip_short --skip_trajopt --device cpu"

BOX_DIR=$(ls -td "$REPO_ROOT"/planning_through_contact/results/benchmark/sugar_box_* 2>/dev/null | head -1 || true)
BOX_SRC="${BOX_DIR}/results_table.pdf"
BOX_DST="$HOME/sugar_box_results_table.pdf"
if [[ -n "$BOX_DIR" && -f "$BOX_SRC" ]]; then
    cp -v "$BOX_SRC" "$BOX_DST"
else
    echo "[warn] could not locate latest sugar_box results_table.pdf under planning_through_contact/results/benchmark/"
fi

run_step "[3/3] Planar pushing tee — table (CPU, max_paths=100)" \
    "bash planning_through_contact/scripts/ptc_benchmark.sh --tee --skip_short --skip_trajopt --device cpu"

TEE_DIR=$(ls -td "$REPO_ROOT"/planning_through_contact/results/benchmark/tee_* 2>/dev/null | head -1 || true)
TEE_SRC="${TEE_DIR}/results_table.pdf"
TEE_DST="$HOME/tee_results_table.pdf"
if [[ -n "$TEE_DIR" && -f "$TEE_SRC" ]]; then
    cp -v "$TEE_SRC" "$TEE_DST"
else
    echo "[warn] could not locate latest tee results_table.pdf under planning_through_contact/results/benchmark/"
fi
...
echo "   - planning_through_contact/results/benchmark/sugar_box_<N>/"
echo "   - planning_through_contact/results/benchmark/tee_<N>/"
