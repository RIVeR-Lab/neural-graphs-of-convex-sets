...
Trains one model family selected by --problem and --planner:
  ...
  quadrotor/nonconvex   -> quadrotor/dataset/quadrotor_gcs_nonconvex.h5  (per-instance regions)
  manipulation/convex   -> manipulation/dataset/manipulation_gcs_convex.h5      (shared regions)
  manipulation/nonconvex-> manipulation/dataset/manipulation_gcs_nonconvex.h5   (shared regions)

Examples:
  python scripts/train_gcs_flow.py --problem manipulation --planner convex --no_wandb
  python scripts/train_gcs_flow.py --problem quadrotor --planner nonconvex --phase flow
"""
...
from model.facet_dataset import (
    FacetGCSDataset,
    FacetRankNetDataset,
    collate_ranknet,
)
from model.facet_lightning import FacetFlowModule, FacetRankNetModule
...
DATASETS = {
    ...
    ("quadrotor", "nonconvex"): ("quadrotor/dataset/quadrotor_gcs_nonconvex.h5", "per_instance"),
    ("manipulation", "convex"): ("manipulation/dataset/manipulation_gcs_convex.h5", "shared"),
    ("manipulation", "nonconvex"): ("manipulation/dataset/manipulation_gcs_nonconvex.h5", "shared"),
}


def _dataset_missing_message(problem: str, h5_path: str) -> str:
    if problem == "quadrotor":
        return (
    ...
    return (
        f"Dataset not found: {h5_path}\n"
        "See README.md (Manipulation) for how to obtain training data:\n"
        "  - Generate: bash manipulation/scripts/manipulation_collect.sh"
    )
...
def main() -> None:
    ...
    p.add_argument("--planner", required=True, choices=("convex", "nonconvex"))
    ...
    if not Path(h5_path).is_file():
        print(_dataset_missing_message(args.problem, h5_path), file=sys.stderr)
        sys.exit(1)

    tag = f"{args.problem}_{args.planner}"
    if args.ckpt_dir:
        ckpt_dir = Path(args.ckpt_dir)
    elif args.problem == "quadrotor":
        ckpt_dir = REPO_ROOT / "quadrotor" / "checkpoints" / tag
    else:
        ckpt_dir = REPO_ROOT / "manipulation" / "checkpoints" / tag
