...
regions, plans joint-space paths with convex and/or nonconvex GCS, and exports Meshcat HTML.

Examples:
  bash scripts/setup_iiwa_models.sh
  python scripts/iiwa_shelf_scenes.py --demo circle
  python scripts/iiwa_shelf_scenes.py --planner nonconvex --demo circle
...
PLANNER_CHOICES = ("convex", "nonconvex", "both")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="GCS planner: convex (configuration-space), nonconvex (GcsTrajectoryOptimization), or both.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.planner in ("nonconvex", "both"):
        print("  Nonconvex GCS...")
        ...
        if nonlinear_traj is None:
            print("Nonconvex GCS planning failed.", file=sys.stderr)
            sys.exit(1)
