...
DATA_ROOT = Path("planning_through_contact/dataset/data")
CHECKPOINT_ROOT = Path("planning_through_contact/checkpoints")
...
def data_dir_for_body(body: str, root: str | Path = DATA_ROOT) -> Path:
...
def flow_checkpoint_path(body: str, root: str | Path = CHECKPOINT_ROOT) -> Path:
    body = validate_body(body)
    return Path(root) / body / f"{flow_checkpoint_name(body)}.ckpt"


def ranker_checkpoint_path(body: str, root: str | Path = CHECKPOINT_ROOT) -> Path:
    body = validate_body(body)
    return Path(root) / body / f"{ranker_checkpoint_name(body)}.ckpt"
