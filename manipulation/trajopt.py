"""Nonlinear GCS planning for IIWA shelf manipulation (GcsTrajectoryOptimization)."""
...
def plan_nonlinear_gcs_segment(
    ...
    *,
    ...
    """
    Plan one joint-space segment with nonlinear GCS.
...
def plan_nonlinear_gcs_path(
    ...
    *,
    ...
    """
    Plan through a sequence of joint-space waypoints with nonlinear GCS.
    ...
    for start_pt, goal_pt in zip(sequence[:-1], sequence[1:]):
        ...
        if traj is None:
            if verbose:
                print(f"  Nonlinear GCS failed between {start_pt} and {goal_pt}", flush=True)
...
def solve_nonlinear_relaxation(graph, source_vertex, target_vertex):
    """Convex relaxation of the nonlinear GCS (labels for flow GNN)."""
...
def plan_nonlinear_and_make_trajectory(
    ...
    *,
    ...
    """Plan a nonlinear GCS path and return (trajectory, solve_time)."""
