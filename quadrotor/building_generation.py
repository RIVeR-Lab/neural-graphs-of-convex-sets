...
EXTERIOR_REGION_MARGIN = CELL_SIZE
QUAD_RADIUS = 0.2
...
def make_exterior_regions(grid_shape):
    """Return the four collision-free strips outside the building perimeter.

    These regions intentionally stop at the outside face of the wall.  They
    should connect to the interior only through explicit door/window passage
    regions generated below.
    """
    x_cells, y_cells = grid_shape
    x_min = -HALF_CELL
    y_min = -HALF_CELL
    ...
    return [
        HPolyhedron.MakeBox(
            [x_min - EXTERIOR_REGION_MARGIN, y_min - EXTERIOR_REGION_MARGIN, Z_MIN],
            [x_max + EXTERIOR_REGION_MARGIN, y_min - WALL_OFFSET, Z_MAX],
        ),
        HPolyhedron.MakeBox(
            [x_min - EXTERIOR_REGION_MARGIN, y_min - WALL_OFFSET, Z_MIN],
            [x_min - WALL_OFFSET, y_max + WALL_OFFSET, Z_MAX],
        ),
        HPolyhedron.MakeBox(
            [x_max + WALL_OFFSET, y_min - WALL_OFFSET, Z_MIN],
            [x_max + EXTERIOR_REGION_MARGIN, y_max + WALL_OFFSET, Z_MAX],
        ),
        HPolyhedron.MakeBox(
            [x_min - EXTERIOR_REGION_MARGIN, y_max + WALL_OFFSET, Z_MIN],
            [x_max + EXTERIOR_REGION_MARGIN, y_max + EXTERIOR_REGION_MARGIN, Z_MAX],
        ),
    ]


def with_repaired_exterior_regions(regions, grid_shape):
    """Replace legacy perimeter regions with the corrected exterior strips."""
    if regions is None or len(regions) < EXTERIOR_REGION_COUNT:
        return regions
    repaired = list(regions)
    repaired[:EXTERIOR_REGION_COUNT] = make_exterior_regions(grid_shape)
    return repaired
...
def compile_sdf(
    ...
    *,
    ...
    regions = make_exterior_regions(grid.shape)
        ...
        for j in range(-1, y_cells + 1):
            ...
            if i >= 0 and j >= 0 and i < x_cells and j < y_cells and grid[i, j] > 0.5:
                ...
                regions.append(HPolyhedron.MakeBox(
                    [xy[0] - (HALF_CELL - WALL_OFFSET), xy[1] - (HALF_CELL - WALL_OFFSET), Z_MIN],
                    [xy[0] + (HALF_CELL - WALL_OFFSET), xy[1] + (HALF_CELL - WALL_OFFSET), Z_MAX]))
            else:
    ...
    for k, (e1, e2) in enumerate(indoor_edges):
        ...
        if sdf_key == "":
            ...
            dx = np.abs(WALL_OFFSET * np.cos(theta) + (half_wall_offset - WALL_OFFSET) * np.sin(theta))
            dy = np.abs(WALL_OFFSET * np.sin(theta) + (half_wall_offset - WALL_OFFSET) * np.cos(theta))
            regions.append(HPolyhedron.MakeBox(
    ...
    tf = RigidTransform(p=np.r_[(start - start) * CELL_SIZE, 0])
    include_static_sdf_at_pose("start_indicator", "models/room_gen/start.sdf", tf)
    tf = RigidTransform(p=np.r_[(goal - start) * CELL_SIZE, 0])
