...
from quadrotor.fastpathplanning.boxes import Box, BoxCollection
from quadrotor.fastpathplanning.polygonal import iterative_planner
from quadrotor.fastpathplanning.smooth import seq_conv_prog
...
def plan(S, p_init, p_term, T, alpha, der_init={}, der_term={}, verbose=True,
    ...
    if box_seq is None:
        ...
        return None, 0.0
    box_seq, traj, length, polygonal_cvxpy_time = iterative_planner(S.B, p_init, p_term, box_seq, verbose)

    polygonal_time = time() - tic
    if verbose:
        print('Polygonal phase terminated in {:.1e}s'.format(polygonal_time))
        print('CVXPY time was {:.1e}s'.format(polygonal_cvxpy_time))
        print('\nSmooth phase:')
    tic = time()
    ...
    path, smooth_cvxpy_time = seq_conv_prog(L, U, trav_times, alpha, initial, final, verbose)

    smooth_time = time() - tic
    if verbose:
        print('Smooth phase terminated in {:.1e}s'.format(smooth_time))
        print('CVXPY time was {:.1e}s'.format(smooth_cvxpy_time))

    return path, polygonal_cvxpy_time + smooth_cvxpy_time
