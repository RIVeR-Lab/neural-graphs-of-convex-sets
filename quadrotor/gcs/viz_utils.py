...
class DelayedTrajectory:
    ...
    def __init__(self, traj: Trajectory, delay: float, *, speed: float = 1.0):
        ...
        self._tf = float(traj.end_time())
        self._start = np.squeeze(traj.value(self._t0))
        self._end = np.squeeze(traj.value(self._tf))

    def _inner_t(self, t: float) -> float:
        if t < self._delay:
            return self._t0
        return min(self._tf, self._t0 + self._speed * (t - self._delay))
    ...
    def EvalDerivative(self, t, derivative_order=1):
        ...
        if t_scalar >= self.end_time():
            dim = self._end.shape[0]
            if derivative_order == 0:
                return self._end.reshape(-1, 1)
            return np.zeros((dim, 1))
