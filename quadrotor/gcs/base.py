...
from quadrotor.gcs.rounding import (
    MipPathExtraction,
...
class BaseGCS:
    ...
    def setRoundingStrategy(self, rounding_fn, **kwargs):
        ...
        if callable(rounding_fn):
            ...
            raise ValueError("Rounding strategy must either be a function or list of functions.")
    ...
    def _make_gcs_options(self, convex_relaxation):
        opts = GraphOfConvexSetsOptions()
        opts.convex_relaxation = convex_relaxation
        opts.max_rounded_paths = 0  # we do rounding manually
        opts.preprocessing = False  # handled separately via removeRedundancies
        if self.solver is not None:
            opts.solver = self.solver
        if self.options is not None:
            opts.solver_options = self.options
        return opts
    ...
    def solveGCS(self, rounding, preprocessing, verbose):
        ...
        # Phase 1: convex relaxation (rounding=True) or MIP (rounding=False)
        phase1_opts = self._make_gcs_options(convex_relaxation=rounding)
        result = self.gcs.SolveShortestPath(self.source, self.target, phase1_opts)
        ...
        if rounding:
            ...
            # Phase 2: solve convex restriction for each rounded path
            restriction_opts = self._make_gcs_options(convex_relaxation=True)
            rounded_results = []
            best_cost = np.inf
            best_path = None
            best_result = None
            ...
            for path_edges in active_edges:
                ...
                rounded_results.append(self.gcs.SolveShortestPath(
                    self.source, self.target, restriction_opts))
                for edge in self.gcs.Edges():
                    edge.ClearPhiConstraints()
