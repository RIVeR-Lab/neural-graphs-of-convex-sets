...
from quadrotor.gcs.base import BaseGCS
...
class BezierGCS(BaseGCS):
    def __init__(self, regions, order, continuity, edges=None, hdot_min=1e-6, full_dim_overlap=False):
        ...
        def _col_pts(mat):
            """Return list of (rows, 1) column slices — required by new Drake API."""
            return [mat[:, i:i+1] for i in range(mat.shape[1])]

        def _scalar_pts(vec):
            return [vec[i:i+1, np.newaxis] for i in range(len(vec))]

        r_basis = BsplineBasis_[Expression](order + 1, order + 1, KnotVectorType.kClampedUniform, 0., 1.)
        h_basis = BsplineBasis_[Expression](order + 1, order + 1, KnotVectorType.kClampedUniform, 0., 1.)

        self.u_r_trajectory = BsplineTrajectory_[Expression](r_basis, _col_pts(u_control))
        self.u_h_trajectory = BsplineTrajectory_[Expression](h_basis, _scalar_pts(u_duration))
        ...
        for ii, jj in edges:
            ...
            for c_con in self.contin_constraints:
                edge.AddConstraint(Binding[Constraint](c_con, np.append(u.x(), v.x())))
    ...
    def SolvePath(self, rounding=False, verbose=False, preprocessing=False):
        best_path, best_result, results_dict = self.solveGCS(rounding, preprocessing, verbose)
        ...
        for edge in best_path:
            if edge.v() == self.target:
                ...
                path_control_points.append(best_result.GetSolution(edge.xv()).reshape(-1, 1))
            ...
            for ii in range(self.order):
                path_control_points.append(edge_path_points[:, ii:ii+1])
