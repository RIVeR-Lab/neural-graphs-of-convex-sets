...
DATA_DIR="${DATA_DIR:-$REPO_ROOT/planning_through_contact/dataset/data/tee}"
...
tmp_dir="$(mktemp -d)"
download_dir="$tmp_dir/tee_pushing"
trap 'rm -rf "$tmp_dir"' EXIT

python3 -m gdown --folder "$DRIVE_URL" -O "$download_dir"

for file in global_features.csv node_features.csv gcs_solutions.h5 tee_pushing.ckpt; do
...
cp "$download_dir"/tee_pushing.ckpt "$CKPT_DIR/tee_pushing.ckpt"

echo "Downloaded tee-pushing dataset to $DATA_DIR"
echo "Downloaded checkpoint to $CKPT_DIR/tee_pushing.ckpt"
