...
class GCSData(Data):
    """Data with graph-level `g`: override __cat_dim__ so batching stacks g to [B, g_dim] instead of concatenating."""

    def __cat_dim__(self, key: str, value: Any, *args: Any, **kwargs: Any) -> Any:
        if key == "g":
            return None  # stack to [B, g_dim]
...
class GCSH5Dataset(Dataset):
    ...
    Each item yields a `torch_geometric.data.Data` with:
      ...
      - y: [E] regression target: binary edge labels (target="discrete") or phi_star (target="sdp")
      - g: [g_dim] global conditioning vector
      - phi_star: [E] teacher relaxation flows (always present in HDF5)
    ...
    def get(self, idx: int) -> Data:  # type: ignore[override]
        ...
        with h5py.File(self.h5_path, "r") as h5:
            ...
            # Regression target: discrete 0/1 (path edges) or SDP relaxation flows
            if self.target == "sdp":
                y_target = phi_star
            else:
                y_target = y
