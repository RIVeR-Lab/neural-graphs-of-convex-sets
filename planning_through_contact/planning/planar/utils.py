...
def create_plan(
    ...
    rounding_profile: dict = {}

    path = None
    if do_rounding:
        t_round_start = time.perf_counter()
        candidate_paths = planner.get_solution_paths(relaxed_result, solver_params, profile=rounding_profile)
        if candidate_paths is not None:
            feasible_paths = planner._get_rounded_paths(
                solver_params, list(candidate_paths), profile=rounding_profile
            )
