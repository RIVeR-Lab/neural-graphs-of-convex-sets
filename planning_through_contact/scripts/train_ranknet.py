...
from planning_through_contact.model.ranknet_datamodule import RankNetDataModule
from planning_through_contact.model.ranknet_lightning_module import RankNetLightningModule
...
def main() -> None:
    ...
    flow_ckpt_path = (
        ...
        else Path(args.ckpt_dir) / "gcs_gnn" / f"{flow_checkpoint_name(body)}.ckpt"
    )
    ...
    callbacks = [
        ModelCheckpoint(
            ...
            mode="min",
            save_top_k=1,
            save_last=f"{ranker_ckpt_filename}_last",
            every_n_epochs=1,
