...
# Checkpoint: best (lowest val loss) epoch*-val_loss*.ckpt if present, else last.ckpt. Override with CKPT_PATH=...
...
if [[ -z "${CKPT_PATH:-}" ]]; then
  # Prefer best checkpoint (lowest val loss) if any epoch*-val_loss*.ckpt exist
  BEST_CKPT=$(for f in "$CKPT_DIR"/epoch*-val_loss*.ckpt; do
    [[ -f "$f" ]] || continue
    v=$(basename "$f" .ckpt | sed -n 's/.*val_loss//p')
    [[ -n "$v" ]] && echo "$v $f"
...
echo "Inference (test split): ckpt=$CKPT_PATH plan_index=$PLAN_INDEX_CSV h5=$H5_PATH rounding_flow=qp"
