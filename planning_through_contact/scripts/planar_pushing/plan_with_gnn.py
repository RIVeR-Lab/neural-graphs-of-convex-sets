...
from model.drake_rounding import predict_edge_flows_for_planner, round_from_predicted_flows
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="planning_through_contact/results/trajectories",
    )
    ...
    for idx, (plan_id, plan) in enumerate(plans_with_ids):
        ...
        if path is not None and path.rounded_result is not None and path.rounded_result.is_success():
            ...
            if args.save_video:
                ...
                pred_mp4 = (traj_dir / "prediction.mp4").resolve()
                print(f"Saved neural inference video -> {pred_mp4}")
                ...
                if args.use_test_plans and h5_path is not None and plan_id is not None and h5_path.exists():
                    ...
                    if (
                        ...
                        gt_mp4 = (traj_dir / "ground_truth.mp4").resolve()
                        print(f"Saved vanilla GCS comparison video -> {gt_mp4}")
