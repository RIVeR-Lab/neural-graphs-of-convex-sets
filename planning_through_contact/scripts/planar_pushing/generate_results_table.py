...
def _fmt_mean_std(
    ...
    *,
    ...
    if median is not None:
        med = median * scale
        return f"{m:.{decimals}f} ± {s:.{decimals}f}\nmed {med:.{decimals}f}{suffix}"
    return f"{m:.{decimals}f} ± {s:.{decimals}f}{suffix}"


def _fmt_paths(mean: Optional[float], std: Optional[float], median: Optional[float] = None) -> str:
    return _fmt_mean_std(mean, std, median, unit="", decimals=1)
...
def _fmt_gap(mean: Optional[float], std: Optional[float], median: Optional[float] = None) -> str:
    if mean is None:
        return "N/A"
...
def build_table(agg: dict[str, Any]) -> tuple[list[str], list[list[str]]]:
    ...
    for prefix in METHOD_PREFIXES:
        ...
        if is_ci:
            ...
            rounding = "—"
            total = _fmt_mean_std(
                agg.get("contact_implicit_total_s_mean"),
                agg.get("contact_implicit_total_s_std"),
                agg.get("contact_implicit_total_s_median"),
            )
            c_round = "N/A"
            gap = "N/A"
        else:
            success = _fmt_success(agg.get(f"{prefix}_success_rate"), n)
            paths_used = _fmt_paths(
                agg.get(f"{prefix}_num_paths_tried_mean"),
                agg.get(f"{prefix}_num_paths_tried_std"),
                agg.get(f"{prefix}_num_paths_tried_median"),
            )
