...
from model.drake_rounding import (
...
from model.ranknet_inference import (
...
def _aggregate(rows: list[dict[str, Any]], max_paths: int, max_paths_short: int) -> dict[str, Any]:
    def _stats(key: str) -> tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    for prefix in gcs_prefixes:
        ...
        for suffix in (
            ...
            m, s, med = _stats(f"{prefix}_{suffix}")
            out[f"{prefix}_{suffix}_mean"] = m
            out[f"{prefix}_{suffix}_std"] = s
            out[f"{prefix}_{suffix}_median"] = med
    ...
    m, s, med = _stats("contact_implicit_total_s")
    out["contact_implicit_total_s_mean"] = m
    out["contact_implicit_total_s_std"] = s
    out["contact_implicit_total_s_median"] = med
...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=str, default="planning_through_contact/results/benchmark")
    ...
    parser.add_argument("--skip_short", action="store_true",
                        help="Skip the *_short (max_paths_short) variants for all methods.")
    ...
    flow_ckpt_path = (
        ...
        else flow_checkpoint_path(body)
    )
    ranker_ckpt_path = (
        Path(args.ranker_ckpt_path)
        if args.ranker_ckpt_path is not None
        else ranker_checkpoint_path(body)
    )
